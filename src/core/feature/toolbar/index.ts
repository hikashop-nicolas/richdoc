...
export function setupToolbar(deps: ToolbarDeps) {
  ...
  // --- Bookmarks, cross-references, captions and links (own module: shared target helpers) --------
  const { insertBookmark, openCaptionDialog, openXrefDialog, openLinkDialog: openLink } = setupReferences({
  ...
  // Special characters insert as plain text, so the picker works on both formats without gating.
  const symbols = setupSymbols({ doc, wrap, mark, getActiveEl, captureSel, restoreSel });
