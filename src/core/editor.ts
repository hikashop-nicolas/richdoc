...
export function createRichEditor(container: HTMLElement, adapter: Adapter, options: EditorOptions = {}): RichEditor {
  ...
  // Comment id allocation, the side panel, edit bookkeeping and click-to-open all live in
  // the comments feature module; mark() is defined here so it can be handed to it.
  const mark = () => {
    dirty = true;
    options.onChange?.();
  ...
  return {
    ...
    async getBytes() {
      ...
      return adapter.write(cleanBody(), editedParts, getEdits(), geometryDirty ? geometry : undefined);
    },
    destroy() {
